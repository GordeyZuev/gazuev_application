...
3. Если у пользователя мало заряда на устройстве, то его предупреждают и пишут, что матч может быть прерван.
4. Проводится жеребьевка, если пользователю это нужно (уточняется в настройках)
...
6. Пользователь видит карточки всех игроков. Короткое нажатие - взаимодейстие с игроком (Замена, предупреждение, забитый гол). Долгое нажатие - просмотр совершенных действий (можно отменить действие).
7. Польватель также видит перед собой список главных тренеров. Для них короткое нажатие - взаимодействие (предупреждение или карта). Долгое нажатие - просмотр полученных предупреждений (можно отменить действие).
8. При выдаче карты, желтой или красной, весь экран телефона загарается в цвет карты.
9. Если Пользователь быстро закрыл экран с картой и игрок не увидел предупреждения, то долгое нажатие на игрока и касание его желтой / красной карты снова ее покажет. 
10. Пользователь может показать флаг Офсайда.
11. Пользователь может увидеть список судей (Главного Арбитра и его ассистентов, если они есть).
12. Пользователь может потянуть часть экрана со временем и счетом вниз для того чтобы растянуть на весь экран.
13. Настройки во время матча (Пауза, звук, включение / отключение блокировки экрана, Справка с правилами и документами)
14. Пользователь (в зависимости от роли, которую он играет в матче - тренер / судья / игрок) может открыть протокол матча или Тактическую доску.
15. В течение матча судья используют специальную кнопку приостановки матча (по причины замены, травмы, празднования забитого мяча), и к концу тайма приложение подсчитает примерное время, которое нужно добавить к тайму. Пользователь может выбрать добавленное время сам.
16. Если счет ничейный, то пользователь может закончить его или выбрать вариант продолжения матча ("Классический режим", "Золотой мяч", "Серия пенальти").
17. Матч можно завершить заранее, приостановить его и доиграть позже, или же, закончить его по окончании времени.

---

После матча:

18. Пользователю Показывается статистика прошедшей встречи и протокол матча (Лучший игррок матча и остальные статистические данные).
19. Пользователь может поделиться матчем через QR Код (Аналогично примеру из сценария "История Матчей").
20. Пользователю предлагается переиграть матч, переиграть его с изменением составов, а также выйти в главное меню.
...
9. Подсчет пройденной дистанции (Пользователь может изменить еденицу измерения расстояния - КМ / Мили).
